...
同時アクセスは DuckDB のファイルロックに従う。別プロセスが書き込み接続を持っている間は、読み取り専用の接続も開けない。`mcp-server-motherduck` は既定で読み取り専用かつ一時接続(`--ephemeral-connections`)で動くため、MCP サーバの起動中でも import できる見込み(実機では未検証)。
